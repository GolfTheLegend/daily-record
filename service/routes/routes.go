...
func Setup(app *fiber.App, store *models.Store, cfg *config.Config) {
	...
	protected.Get("/daily-records/status", dailyHandler.GetStatusDailyRecords)
	protected.Get("/daily-records/:id", dailyHandler.GetDailyRecordByID)
	...
	protected.Put("/daily-records/check-list/:id", dailyHandler.UpdateDailyCheckList)
